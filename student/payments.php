...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $pending = max(
        0.0,
        (float) $fee['amount'] - (float) $fee['discount'] - (float) $fee['paid_amount']
    );
...
$feeStmt = db()->prepare(
    'SELECT sf.id, sf.category, sf.amount, sf.discount, sf.paid_amount, sf.due_date,
     ...
     WHERE sf.student_id = ?
       AND sf.paid_amount < sf.amount - sf.discount
...
$receiptStmt = db()->prepare(
    'SELECT p.id, p.reference, p.amount, p.provider, p.status, p.paid_at,
            p.student_fee_id, sf.category, rc.receipt_no, rc.issued_at
     FROM payments p
    ...
    <div class="row g-3 mb-4">
        <div class="col-sm-6 col-xl-3">
            <div class="card border-0 shadow-sm h-100"><div class="card-body">
                <div class="small text-muted">Outstanding fees</div>
                <div class="h3 fw-bold mb-0">₹<?= e(number_format($totalPending, 2)) ?></div>
            </div></div>
        </div>
        <div class="col-sm-6 col-xl-3">
            <div class="card border-0 shadow-sm h-100"><div class="card-body">
        ...
        <div class="col-sm-6 col-xl-3">
            <div class="card border-0 shadow-sm h-100"><div class="card-body">
                <div class="small text-muted">Payment count</div>
                <div class="h3 fw-bold mb-0"><?= count($payments) ?></div>
            </div></div>
        </div>
        <div class="col-sm-6 col-xl-3">
            <div class="card border-0 shadow-sm h-100"><div class="card-body">
                <div class="small text-muted">Student ID</div>
                <div class="h5 fw-bold mb-0"><?= e($student['enrollment_no'] ?: '—') ?></div>
            </div></div>
        </div>
    </div>
                                ...
                                <div class="d-flex justify-content-between align-items-start gap-3">
                                    <div>
                                        ...
                                        <div class="small text-muted mt-1">
                                            ...
                                            <?php if ($fee['due_date']): ?>
                                                · Due <?= e(date('d M Y', strtotime((string) $fee['due_date']))) ?>
                                            <?php endif; ?>
                                        </div>
                                    </div>
            ...
            <div class="table-responsive">
                <table class="table table-hover align-middle mb-0">
                    ...
                    <?php foreach ($payments as $payment):
                        $success = strtolower((string) $payment['status']) === 'success';
                    ?>
                        <tr>
