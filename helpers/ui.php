...
function page_top(string $title = 'DCOS 2077', bool $fluid = true): void
{
    ...
    <head>
        ...
        <link rel="stylesheet" href="<?= e($base) ?>/assets/css/vendor/fontawesome.min.css">
        <link rel="stylesheet" href="<?= e($base) ?>/assets/css/vendor/bootstrap.min.css">
        <script defer src="<?= e($base) ?>/assets/js/vendor/bootstrap.bundle.min.js"></script>
    ...
    <div class="app-shell">
        ...
        <aside class="sidebar" id="sidebar">
            <div class="brand-wrap"><div class="brand-mark">C</div><div><div class="brand">DCOS <span>2077</span></div><div class="brand-mini">DIGITAL CAMPUS OS</div></div></div>
            <?php nav_menu(); ?>
        </aside>
        <section class="content">
            <nav class="topbar" aria-label="Application navigation">
                <button class="nav-toggle" type="button" data-sidebar-toggle aria-label="Open navigation" aria-controls="sidebar" aria-expanded="false">☰</button>
                ...
                <div class="ms-auto d-flex align-items-center gap-2"><button class="icon-btn" type="button" data-theme-toggle aria-label="Toggle theme">◐</button><a class="icon-btn" href="<?= e($base) ?>/student/notifications.php" aria-label="Notifications">♢</a><div class="user-chip d-none d-sm-flex"><strong><?= e((string)($u['name'] ?? 'Guest')) ?></strong><small><?= e((string)($u['role_name'] ?? '')) ?></small></div><a href="<?= e($base) ?>/auth/logout.php" class="btn btn-sm btn-outline-danger">Logout</a></div>
            </nav>
...
function nav_menu(): void
{
    ...
    foreach ($items as $path=>$label) { $active = str_contains($current,$path) ? ' active' : ''; echo '<a class="nav-item'.$active.'" href="'.e($base.$path).'">◉ <span>'.e($label).'</span></a>'; }
    echo '<div class="nav-divider"></div><a class="nav-item" href="'.e($base.'/public/verification.php').'">◈ <span>Certificate Verification</span></a>';
}
