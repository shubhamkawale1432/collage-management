    ...
    <div class="app-shell">
        ...
        <aside class="sidebar" id="sidebar">
            <div class="brand-wrap">
                ...
                <div>
                    <div class="brand">DCOS <span>2077</span></div>
                    <div class="brand-mini">DIGITAL CAMPUS OS</div>
                </div>
            </div>
            <?php nav_menu(); ?>
        </aside>
        ...
        <section class="content">
            <nav class="topbar" aria-label="Application navigation">
                <button class="nav-toggle" type="button" data-sidebar-toggle aria-label="Open navigation" aria-controls="sidebar" aria-expanded="false">
                    <i class="fa-solid fa-bars" aria-hidden="true"></i>
                </button>

                <div class="global-search">
                    <label class="visually-hidden" for="globalSearch">Search</label>
                    <input id="globalSearch" class="form-control form-control-sm" type="search" placeholder="Search authorized records…" autocomplete="off">
                </div>

                <div class="ms-auto d-flex align-items-center gap-2">
                    <button class="icon-btn" type="button" data-theme-toggle aria-label="Toggle theme" title="Toggle theme">
                        <i class="fa-solid fa-circle-half-stroke" aria-hidden="true"></i>
                    </button>
                    <a class="icon-btn" href="<?= e($base) ?>/student/notifications.php" aria-label="Notifications" title="Notifications">
                        <i class="fa-regular fa-bell" aria-hidden="true"></i>
                    </a>
                    <div class="user-chip d-none d-sm-flex">
                        <strong><?= e((string)($u['name'] ?? 'Guest')) ?></strong>
                        <small><?= e((string)($u['role_name'] ?? '')) ?></small>
                    </div>
                    <a href="<?= e($base) ?>/auth/logout.php" class="btn btn-sm btn-outline-danger">Logout</a>
                </div>
            </nav>

            <main class="container-fluid py-4<?= $fluid ? '' : ' px-0' ?>">
                <div class="d-flex justify-content-between align-items-center mb-4">
                    <div>
                        <h1 class="page-title mb-1"><?= e($title) ?></h1>
                        <div class="small text-muted"><?= e(date('d M Y · H:i')) ?></div>
                    </div>
                </div>
    <?php
}
...
function page_bottom(): void
{
    ...
    ?>
            ...
            <footer class="text-center small text-muted py-4">
                Digital College Operating System · <?= e((string)($app['name'] ?? 'DCOS 2077')) ?>
            </footer>
        </section>
    </div>
    </body>
...
function nav_menu(): void
{
    ...
    $items = $sets[$role] ?? ['/dashboard.php'=>'Dashboard'];
    $current = (string)($_SERVER['REQUEST_URI'] ?? '');

    foreach ($items as $path => $label) {
        $href = $base . $path;
        $active = str_contains($current, $path) ? ' active' : '';
        echo '<a class="nav-item' . $active . '" href="' . e($href) . '"><i class="fa-solid fa-circle" aria-hidden="true"></i><span>' . e($label) . '</span></a>';
    }

    echo '<div class="nav-divider"></div>';
    echo '<a class="nav-item" href="' . e($base . '/public/verification.php') . '"><i class="fa-solid fa-shield-halved" aria-hidden="true"></i><span>Certificate Verification</span></a>';
}

function kpi(string $label, string|int|float $value, string $icon = '◉'): void
{
    echo '<div class="col-sm-6 col-xl-3"><div class="kpi-card"><div><span class="small text-muted">' . e($label) . '</span><div class="kpi-value">' . e((string)$value) . '</div></div><div class="kpi-icon" aria-hidden="true">' . e($icon) . '</div></div></div>';
}
...
function flash(): void
{
    ...
    $type = in_array((string)($flash[0] ?? ''), ['success','danger','warning','info'], true) ? (string)$flash[0] : 'info';
    $message = (string)($flash[1] ?? '');
    echo '<div class="alert alert-' . e($type) . '" role="alert">' . e($message) . '</div>';
}
